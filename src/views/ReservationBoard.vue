...
const orderList = computed(() => {
  return Object.entries(ordersMap.value)
    .map(([key, order]) => {
...
const handleCellClick = (timeSlot: string, resourceId: number) => {
  ...
  const order = ordersMap.value[key]
...
const handleApprove = () => {
  if (selectedCell.value) {
    ...
    if (ordersMap.value[key]) {
      ordersMap.value[key].status = 'confirmed'
    }
...
const handleWriteOff = () => {
  if (selectedCell.value) {
    ...
    const key =
      ...
      `${selectedCell.value.date}_${selectedCell.value.timeSlot}_${selectedCell.value.resourceId}`
    if (ordersMap.value[key]) {
      ordersMap.value[key].status = 'completed'
    }
    alert('核销成功，用户已入场')
  }
}
...
const getCellStatus = (timeSlot: string, resourceId: number) => {
  const key = `${currentDate.value}_${timeSlot}_${resourceId}`
  const order = ordersMap.value[key]
...
<template>
  <div class="reservation-board">
        ...
        <tbody>
          <tr v-for="timeSlot in projectConfig.timeSlots" :key="timeSlot">
            ...
            <td v-for="res in projectConfig.subResources" :key="res.id" class="cell-wrapper">
              <div
                class="matrix-cell"
                :class="getCellStatus(timeSlot, res.id)"
                @click="handleCellClick(timeSlot, res.id)"
              >
                <template v-if="getCellStatus(timeSlot, res.id) === 'free'">
                  <span class="free-text">空闲</span>
                </template>
                <template v-else>
                  <div class="order-info">
                    <div class="user-name">
                      {{ ordersMap[`${currentDate}_${timeSlot}_${res.id}`].name }}
                    </div>
                    <div
                      class="order-type"
                      v-if="ordersMap[`${currentDate}_${timeSlot}_${res.id}`].type === 'sku_order'"
                    >
                      电商单
                    </div>
                    <div class="order-type free" v-else>独立预约</div>
                  </div>
                </template>
              </div>
            </td>
    ...
    <div class="detail-drawer" :class="{ 'is-open': isDrawerVisible }">
      ...
      <div class="drawer-body" v-if="selectedCell">
        ...
        <div class="info-group">
          <div class="info-label">占用资源</div>
          <div class="info-value">{{ selectedCell.resource }}</div>
        </div>
        <div class="divider"></div>
        <div class="info-group">
          <div class="info-label">客户姓名</div>
          <div class="info-value">{{ selectedCell.name }}</div>
        </div>
        <div class="info-group">
          <div class="info-label">联系电话</div>
          <div class="info-value">{{ selectedCell.phone }}</div>
        </div>
        <div class="divider"></div>
        <div class="info-group">
          <div class="info-label">订单来源</div>
          <div class="info-value">
            <span class="tag" :class="selectedCell.type">{{
              selectedCell.type === 'sku_order' ? '商品购买 (SKU关联)' : '独立预约登记'
            }}</span>
          </div>
        </div>
        <div class="info-group">
          ...
          <div class="info-value">
            <span class="status-tag" :class="selectedCell.status">
              {{
                selectedCell.status === 'confirmed'
                  ? '已预订待核销'
        ...
        <div class="drawer-actions">
          <button
            ...
            v-if="selectedCell.status === 'confirmed'"
            @click="handleWriteOff"
          >
            确认核销入场
          </button>

          <template v-if="selectedCell.status === 'pending'">
            <button class="btn btn-primary" @click="handleApprove">同意预约 (通过审核)</button>
            <button class="btn btn-danger-outline" @click="handleReject">拒绝预约</button>
          </template>

          <button
            class="btn btn-default"
            v-if="['pending', 'confirmed'].includes(selectedCell.status)"
            @click="handleCancel"
          >
            取消订单
          </button>
